...
CGFloat const SVInfiniteScrollingViewHeight = 49;
...
@property (nonatomic, readwrite) BOOL withoutAdjustBottomInset;
...
- (void)addInfiniteScrollingWithActionHandler:(void (^)(void))actionHandler {
    ...
    if(!self.infiniteScrollingView) {
        ...
        view.withoutAdjustBottomInset = NO;
        self.infiniteScrollingView = view;
        self.showsInfiniteScrolling = YES;
    }
}

- (void)addInfiniteScrollingWithoutAdjustBottomInset {
    ...
    if(!self.infiniteScrollingView) {
        ...
        view.originalBottomInset = self.contentInset.bottom;
        view.withoutAdjustBottomInset = YES;
        self.infiniteScrollingView = view;
        self.showsInfiniteScrolling = YES;
    }
}

- (void)refreshInfiniteScrollingOriginY:(CGFloat)originY {
    if (self.infiniteScrollingView.withoutAdjustBottomInset == YES) {
        CMTLog(@"\nscrollView.infiniteScrollingView.originY: %g", originY);
        self.infiniteScrollingView.originY = originY;
        self.infiniteScrollingView.frame = CGRectMake(0, originY, self.infiniteScrollingView.bounds.size.width, SVInfiniteScrollingViewHeight);
    }
    else {
...
- (void)setShowsInfiniteScrolling:(BOOL)showsInfiniteScrolling {
    ...
    if(!showsInfiniteScrolling) {
      if (self.infiniteScrollingView.isObserving) {
        ...
        if (self.infiniteScrollingView.withoutAdjustBottomInset == NO) {
      ...
      if (!self.infiniteScrollingView.isObserving) {
        ...
        if (self.infiniteScrollingView.withoutAdjustBottomInset == YES) {
            self.infiniteScrollingView.frame = CGRectMake(0, self.infiniteScrollingView.originY, self.infiniteScrollingView.bounds.size.width, SVInfiniteScrollingViewHeight);
        }
        else {
            self.infiniteScrollingView.frame = CGRectMake(0, self.contentSize.height, self.infiniteScrollingView.bounds.size.width, SVInfiniteScrollingViewHeight);
        }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {    
    if([keyPath isEqualToString:@"contentOffset"])
        ...
        [self layoutSubviews];
        if (self.withoutAdjustBottomInset == YES) {
            self.frame = CGRectMake(0, self.originY, self.bounds.size.width, SVInfiniteScrollingViewHeight);
        }
        else {
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != SVInfiniteScrollingStateLoading && self.enabled) {
        ...
        if (self.withoutAdjustBottomInset == YES) {
            scrollViewContentHeight += self.scrollView.contentInset.bottom;
        }
