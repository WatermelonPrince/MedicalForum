...
- (void)sendButtonTappedWithLog:(NSString *)log
{
    ...
    NSString *appVersionString = [NSString stringWithFormat:@"%@.%@", version, build];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) {
        cell.textLabel.text = @"发送Email";
    }
    else if (indexPath.row == 1) {
        cell.textLabel.text = @"切换服务器";
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // 切换服务器
        ...
        NSString *serverTitle, *serverMessage;
        @try {
            [CMTClient changeClientServer];
            NSURL *clientBaseURL = [CMTCLIENT baseURL];
            ...
            if ([clientBaseURL isEqual:debugServer.baseURL]) {
                serverTitle = @"切换到测试服务器";
            }
            else if ([clientBaseURL isEqual:releaseServer.baseURL]) {
                serverTitle = @"切换到正式服务器";
            }
            else {
                serverTitle = @"默认服务器地址错误";
            }
            serverMessage = clientBaseURL.absoluteString;
        }
        @catch (NSException *exception) {
            serverTitle = @"切换服务器失败 请查看log中的'CLIENT Change Server Exception'信息";
            CMTLogError(@"CLIENT Change Server Exception: %@", exception);
        }
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:serverTitle message:serverMessage delegate:nil cancelButtonTitle:@"关闭" otherButtonTitles:nil, nil];
        [alert show];
        CMTLogDebug(@"%@ %@", serverTitle, serverMessage ?: @"");
    }
    // 清空缓存
    else if (indexPath.row == 2) {
        NSString *clearTitle, *clearMessage;
        if (PATH_CLEAR == YES) {
            clearTitle = @"清空缓存成功";
            clearMessage = nil;
        }
            ...
            clearTitle = @"清空缓存失败";
            clearMessage = @"请卸载App 并重新安装 以清空缓存";
        }
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:clearTitle message:clearMessage delegate:nil cancelButtonTitle:@"关闭" otherButtonTitles:nil, nil];
        [alert show];
        CMTLogDebug(@"%@ %@", clearTitle, clearMessage ?: @"");
    }
}
