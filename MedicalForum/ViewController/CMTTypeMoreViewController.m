...
@interface CMTTypeMoreViewController ()<UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource>
...
@implementation CMTTypeMoreViewController
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    self.mTfSearch.placeholder = self.placeHold;
...
- (void)initData:(NSDictionary *)dic
{
    ...
    @weakify(self);
    [self.rac_deallocDisposable addDisposable:[[CMTCLIENT searchPostInDepart:dic]subscribeNext:^(NSArray * result) {
        @strongify(self);
...
- (void)pullToGetMore:(id)sender
{
    ...
    {
        @weakify(self);
        [self.rac_deallocDisposable addDisposable:[[CMTCLIENT searchPostInDepart:pDic]subscribeNext:^(NSArray * more) {
            @strongify(self);
            DEALLOC_HANDLE_SUCCESS
            CMTLog(@"%@",more);
...
- (UIView *)mBaseView
{
    if (!_mBaseView)
    {
        ...
        _mBaseView.userInteractionEnabled = NO;
    }
    return _mBaseView;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    @catch (NSException *exception) {
        CMTLog(@"self.ArrResult.object is not kind of CMTPost");
    }
}
